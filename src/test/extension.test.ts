...
suite('Extension Test Suite', () => {
	...
	test('should activate successfully', () => {
		...
		assert.ok(mockContext.subscriptions[0] instanceof DockerBehatTestController);

		// Verify output channel was created
		sinon.assert.calledOnce(vscode.window.createOutputChannel as sinon.SinonStub);
		sinon.assert.calledWith(vscode.window.createOutputChannel as sinon.SinonStub, 'Behat Docker');

		// Verify output channel messages
		sinon.assert.calledWith(mockOutputChannel.appendLine, 'Behat Docker extension is now active');
		sinon.assert.calledWith(mockOutputChannel.appendLine, 'Behat Docker extension initialization complete');

		// Verify test controller was created
		sinon.assert.calledOnce(vscode.tests.createTestController as sinon.SinonStub);
		sinon.assert.calledWith(vscode.tests.createTestController as sinon.SinonStub, 'behatDocker');
	});
